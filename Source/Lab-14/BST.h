...
private:
  ...
  public:
    ...
    BinNode(DataType item) : data(item), left(NULL), right(NULL) {}
  ...
  void search2(const DataType &item, bool &found, BinNodePointer &locptr,
...
template <typename DataType> inline BST<DataType>::BST() : myRoot(NULL) {}

//--- Definition of empty()
template <typename DataType> inline bool BST<DataType>::empty() const {
  return myRoot == NULL;
}
...
template <typename DataType> void BST<DataType>::remove(const DataType &item) {
...
// For destructor
template <typename T> void BST<T>::BinNode::selfDestruct() {
  if (left)
    left->selfDestruct();
  if (right)
    right->selfDestruct();
...
template <typename T> void BST<T>::BinNode::copyTo(BST::BinNode *node) const {
  node->data = this->data;
  if (left) {
    if (node->left == NULL)
...
template <> void BST<Token>::insert(const Token &word) {
...
template <typename T> ostream &operator<<(ostream &out, const BST<T> &bst) {
  bst.graph(out);
  return out;
}
