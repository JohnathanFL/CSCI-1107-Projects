...
template <typename T> class CircularQ {
public:
...
template <typename T> ostream &operator<<(ostream &out, const CircularQ<T> &q);

//==============================================================================
// CLASS FUNCTION DEFS
//==============================================================================

template <typename T> CircularQ<T>::CircularQ() { myBack = NULL; }

// Since it's just 2 statements, should really be inlined...
template <typename T> CircularQ<T>::CircularQ(const CircularQ<T> &orig) {
  myBack = NULL;
...
template <typename T> CircularQ<T>::~CircularQ() {
  if (myBack) {
...
template <typename T> bool CircularQ<T>::empty() const {
  return myBack == NULL; // If myBack is null we have no nodes, so empty.
}

template <typename T> void CircularQ<T>::enqueue(const T &value) {
  if (myBack) { // !empty
...
template <typename T> void CircularQ<T>::display(ostream &out) const {
  if (myBack) {
    Node *cur = myBack->next;
    do {
      out << cur->data << " ";
...
template <typename T> T CircularQ<T>::front() const {
  return myBack->next->data;
}

template <typename T> void CircularQ<T>::dequeue() {
